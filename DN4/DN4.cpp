...
int main() {
  ...
  for (int ii=0; ii<2000; ii++)
    {
  // Zunanja iteracija (ne paraleliziramo)
  // Razlog: Gauss–Seidel ima podatkovne odvisnosti med zaporednimi T(j)
  for (int jj = 0; jj < n; jj++)
  {
    double d = b[jj];

    // NOTRANJO VSOTO LAHKO PARALELIZIRAMO
    // Gre za redukcijo, zato uporabimo reduction(+:d)
    ...
    // Posodobitev trenutnega elementa
    T[jj] = d / A[jj][jj];
  }
...
/*
  Gauss–Seidel metoda ni neposredno paralelizabilna po indeksu jj,
  ker se pri računu T[jj] uporabljajo že posodobljene vrednosti T[0..jj-1].
  To pomeni, da obstaja podatkovna odvisnost med zaporednimi iteracijami.

  Možna je delna paralelizacija notranje zanke (po ii2),
  saj gre za seštevanje (redukcijo), ki ne vpliva na zaporedje
  posodabljanja T[jj].

  Čeprav je notranjo zanko matematično možno paralelizirati,
  je v praksi to neučinkovito, saj je število operacij zelo majhno (n=256),
  overhead ustvarjanja OpenMP niti pa veliko večji od prihranka.

  Posledično je paralelizirana verzija veliko počasnejša od serijske.
*/
